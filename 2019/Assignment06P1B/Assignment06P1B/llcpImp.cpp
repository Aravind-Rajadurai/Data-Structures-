...
void SortedMergeRecur(Node*& aHead, Node*& bHead, Node*& cHead)
{
    if (aHead == 0 && bHead == 0)
    {
        return; // Exiting the function if both lists are empty
    }
    else
    {
        if (aHead != 0 && (bHead == 0 || aHead->data <= bHead->data))
        {
            
            cHead = aHead; // Adding the Node in a to c
            aHead = aHead->link; // Moving down a list
            cHead->link = 0; // Clearing the lingering link
            SortedMergeRecur(aHead, bHead, cHead->link); // Recursive call
            
        }
        else
        {
            cHead = bHead; // Adding b Node to c list
            bHead = bHead->link; // Moving down the b list
            cHead->link = 0; // Clearing the lingering link
            SortedMergeRecur(aHead, bHead, cHead->link); // Recursive call
        }
    }
